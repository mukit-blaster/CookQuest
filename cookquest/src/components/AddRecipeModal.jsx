...
const AddRecipeModal = () => {
  return (
    <>
      {/* Overlay */}
      <div className="overlay hidden fixed inset-0 bg-black/70 backdrop-blur-sm z-40"></div>

      {/* Modal */}
      <div className="add-recipe-window hidden fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 
        w-full max-w-lg bg-[#26130d]/95 rounded-2xl shadow-2xl border border-[#ff914d]/30 p-8 z-50">
        
        {/* Close Button */}
        <button className="absolute top-4 right-4 text-3xl text-gray-400 hover:text-[#ff914d] transition">
          &times;
        </button>
        ...
        <form className="grid md:grid-cols-2 gap-6 text-[#fef9f6]">
          ...
          {/* Submit Button */}
          <button className="col-span-2 mt-4 bg-gradient-to-r from-[#ff914d] via-[#ff4d4d] to-[#facc15] 
